...
export const EXAMPLE_CODEBLOCK_CONTENT =
...
export const EXAMPLE_INLINE_CODE =
...
export function convertSettings(
	...
	if (typeof settings?.version === "undefined") return settingsClear();
...
function semverNewer(newVersion: string, oldVersion: string): boolean {
	return (
		newVersion.localeCompare(oldVersion, undefined, { numeric: true }) === 1
	);
}

function settingsVersionUpdate(
	settings: CodeStylerSettings,
	themeUpdater: (theme: CodeStylerTheme) => CodeStylerTheme = (theme) =>
		theme,
	otherSettingsUpdater: (
		settings: CodeStylerSettings
	) => CodeStylerSettings = (settings) => settings,
	redirectLanguagesUpdater: (
		redirectLanguages: Record<string, { colour?: Colour; icon?: string }>
	) => Record<string, { colour?: Colour; icon?: string }> = (
		redirectLanguages
	) => redirectLanguages
): CodeStylerSettings {
	...
	settings.currentTheme = structuredClone(
		settings.themes[settings.selectedTheme]
	);
	settings.redirectLanguages = redirectLanguagesUpdater(
		settings.redirectLanguages
	);
	settings = otherSettingsUpdater(settings);
	settings.version =
		Object.keys(settingsUpdaters).find(
...
> = {
	...
	"1.1.4": (settings) =>
		settingsVersionUpdate(
			...
			(theme) => {
				// To 1.1.5
				theme.settings.header.externalReference = structuredClone(
					THEME_DEFAULT_SETTINGS.header.externalReference
				);
				theme.colours.light.header.externalReference = structuredClone(
					THEME_FALLBACK_COLOURS.header.externalReference
				);
				theme.colours.dark.header.externalReference = structuredClone(
					THEME_FALLBACK_COLOURS.header.externalReference
				);
				return theme;
			},
...
export const EXTERNAL_REFERENCE_PATH = "/plugins/code-styler-2/reference-files/";
...
const PRISM_LANGUAGES: { [key: string]: string } = {
...
export const EXECUTE_CODE_SUPPORTED_LANGUAGES = [
	...EXECUTE_CODE_LANGUAGE_ALIASES,
	...EXECUTE_CODE_CANONICAL_LANGUAGES,
];
