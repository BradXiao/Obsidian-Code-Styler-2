...
export async function readingViewCodeblockDecoratingPostProcessor(
	...
	if (
		!sourcePath ||
		!element ||
		(frontmatter ?? cache?.frontmatter)?.["code-styler-ignore"] === true
	)
		return;

	editingEmbeds =
		editingEmbeds || Boolean(element.matchParent(".cm-embed-block"));
	...
	const codeblockSectionInfo: MarkdownSectionInformation | null =
		getSectionInfo(codeblockPreElements[0]);
	if (codeblockSectionInfo && specific && !editingEmbeds)
		await renderSpecificReadingSection(
		...
		await retriggerProcessor(
			element,
			{ sourcePath, getSectionInfo, frontmatter },
			plugin,
			editingEmbeds
		);
	else
...
export async function readingViewInlineDecoratingPostProcessor(
	element: HTMLElement,
	{
		sourcePath,
	}: {
	...
	if (!sourcePath || !element) return;
	for (const inlineCodeElement of Array.from(
		element.querySelectorAll(":not(pre) > code")
	)) {
...
export function destroyReadingModeElements(): void {
	document
		.querySelectorAll(".code-styler-pre-parent")
		.forEach((codeblockPreParent) => {
			codeblockPreParent.classList.remove("code-styler-pre-parent");
		});
	[
		...Array.from(
			document.querySelectorAll(
				"pre.code-styler-pre div[class^='code-styler-header-container']"
			)
		),
		...Array.from(
			document.querySelectorAll(
		...
		...Array.from(
			document.querySelectorAll(
				":not(pre) > code span.code-styler-inline-opener"
			)
		),
	].forEach((element) => element.remove());
	document
		.querySelectorAll("pre.code-styler-pre")
		.forEach((codeblockPreElement: HTMLElement) => {
	...
	document
		.querySelectorAll("pre > code ~ code.language-output")
		.forEach((executeCodeOutput: HTMLElement) => {
	...
	document
		.querySelectorAll("pre > code:nth-of-type(1)")
		.forEach((codeblockCodeElement: HTMLElement) => {
			...
			codeblockCodeElement.innerHTML =
				Array.from(
					codeblockCodeElement.querySelectorAll(
						'code > [class*="code-styler-line"]'
					)
				)
					.reduce(
						(
							...
							if (codeblockLineText)
								reconstructedCodeblockLines.push(
									codeblockLineText.innerHTML
								);
	...
	document
		...
		.forEach((inlineCodeElement: HTMLElement) => {
			...
			inlineCodeElement.innerText =
				inlineCodeElement.getAttribute("parameters") +
				inlineCodeElement.innerText;
		});
}
...
export async function renderSpecificReadingSection(
	...
	const codeblocksParameters = (
		await parseCodeblockSource(
			Array.from(
				{
					length:
						codeblockSectionInfo.lineEnd -
						codeblockSectionInfo.lineStart +
						1,
				},
				(_, num) => num + codeblockSectionInfo.lineStart
			).map(
				(lineNumber) =>
...
async function renderSettings(
	...
	const codeblocksParameters = (
		await parseCodeblockSource(
			sourcePath.substring(SETTINGS_SOURCEPATH_PREFIX.length).split("\n"),
			plugin
		)
	).codeblocksParameters;
	await remakeCodeblocks(
...
async function retriggerProcessor(
	element: HTMLElement,
	context: {
		sourcePath: string;
		getSectionInfo: (
	...
	if (
		element.matchParent("div.block-language-dataviewjs") &&
		isCodeblockIgnored(
			"dataviewjs",
	...
	if (editingEmbeds || !element.classList.contains("admonition-content")) {
		const contentEl =
			element.matchParent(".view-content") ??
			element.matchParent("div.print");
		if (contentEl !== null)
			await readingViewCodeblockDecoratingPostProcessor(
				contentEl as HTMLElement,
				context,
				plugin,
				editingEmbeds
			); // Re-render whole document
	}
}
...
async function remakeCodeblocks(
	...
	if (codeblockPreElements.length !== codeblocksParameters.length) return;
	for (const [key, codeblockPreElement] of Array.from(
		codeblockPreElements
	).entries()) {
		const codeblockParameters = codeblocksParameters[key];
		const codeblockCodeElement: HTMLElement | null =
			codeblockPreElement.querySelector("pre > code");
		if (!codeblockCodeElement) return;
		if (
			Array.from(codeblockCodeElement.classList).some((className) =>
		...
		if (
			skipStyled &&
			codeblockCodeElement.querySelector(
				"code [class*='code-styler-line']"
			)
		)
			continue;
		if (
			isLanguageIgnored(
				...
				plugin.settings.excludedLanguages
			) ||
			codeblockParameters.ignore
		)
			continue;
		await remakeCodeblock(
...
async function remakeCodeblock(
	...
	codeblockPreElement.classList.add(
		...getPreClasses(codeblockParameters, dynamic)
	);
	codeblockPreElement.setAttribute(
		"defaultFold",
		codeblockParameters.fold.enabled.toString()
	);
	if (codeblockPreElement.parentElement)
		codeblockPreElement.parentElement.classList.add(
			"code-styler-pre-parent"
		);

	if (!codeblockCodeElement.querySelector("code [class*='code-styler-line']"))
		// Ignore styled lines
		decorateCodeblockLines(
			codeblockCodeElement,
			codeblockParameters,
			sourcePath,
			plugin
...
async function remakeInlineCode(
	inlineCodeElement: HTMLElement,
	plugin: CodeStylerPlugin
): Promise<void> {
	if (
		!plugin.settings.currentTheme.settings.inline.syntaxHighlight ||
		inlineCodeElement.classList.contains("code-styler-inline")
	)
		return;
	const inlineCodeText = inlineCodeElement.innerText;
	const { parameters, text } = parseInlineCode(inlineCodeText);
	if (parameters) {
		inlineCodeElement.innerHTML = await getHighlightedHTML(
			parameters,
			text,
			plugin
		);
		inlineCodeElement.innerHTML =
			inlineCodeElement.innerHTML + "&ZeroWidthSpace;";
		inlineCodeElement.classList.add("code-styler-inline");
		const parameterString = inlineCodeText.substring(
			0,
			inlineCodeText.lastIndexOf(text)
		);
		...
		if (parameters.icon || parameters.title)
			inlineCodeElement.insertBefore(
				createInlineOpener(parameters, plugin.languageIcons),
				inlineCodeElement.childNodes[0]
			);
...
async function getCodeblockPreElements(
	element: HTMLElement,
	specific: boolean,
	editingEmbeds: boolean
	...
	if (!editingEmbeds && !specific)
		codeblockPreElements = Array.from(
			element.querySelectorAll(
				".markdown-reading-view pre:not(.frontmatter)"
			)
		);
	else if (editingEmbeds && !specific)
		codeblockPreElements = Array.from(
			element.querySelectorAll(
				".markdown-source-view .cm-embed-block pre:not(.frontmatter)"
			)
		);
	else if (!editingEmbeds && specific) {
		codeblockPreElements = Array.from(
		...
		const admonitionCodeElement = codeblockPreElements?.[0]?.querySelector(
			'pre:not([class]) > code[class*="language-ad-"]'
		);
	...
	} else codeblockPreElements = [];
	return codeblockPreElements;
}

async function getCodeblocksParameters(
	sourcePath: string,
	cache: CachedMetadata | null,
	plugin: CodeStylerPlugin,
	editingEmbeds: boolean
	...
	if (typeof cache?.sections !== "undefined") {
		for (const section of cache.sections) {
			if (
				...
				const parsedCodeblocksParameters = await parseCodeblockSource(
				...
				if (!editingEmbeds || parsedCodeblocksParameters.nested)
					codeblocksParameters = codeblocksParameters.concat(
						parsedCodeblocksParameters.codeblocksParameters
					);
			}
		}
	} else console.error(`Metadata cache not found for file: ${sourcePath}`);
...
function insertHeader(
	...
	const headerContainer = createHeader(
		codeblockParameters,
		plugin.settings.currentTheme.settings,
		sourcePath,
		plugin
	);
	if (dynamic)
		...
		}); // Add listener for header folding on click
	codeblockPreElement.insertBefore(
		headerContainer,
		codeblockPreElement.childNodes[0]
	);
}

export function readingDocumentFold(contentEl: HTMLElement, fold?: boolean) {
	const codeblockPreElements = contentEl.querySelectorAll(
		"pre.code-styler-pre"
	);
	if (typeof fold === "undefined")
		//Return all blocks to original state
		codeblockPreElements.forEach((codeblockPreElement: HTMLElement) => {
			toggleFold(
				codeblockPreElement,
				(codeblockPreElement.getAttribute("defaultFold") ?? "false") ===
					"true"
			);
		});
	//Fold or unfold all blocks
		...
		codeblockPreElements.forEach((codeblockPreElement: HTMLElement) => {
			toggleFold(codeblockPreElement, fold);
		});
}
...
async function toggleFold(
	...
	if (
		codeblockPreElement.firstElementChild?.classList?.contains(
	...
	codeblockPreElement
		...
		.forEach((codeblockCodeElement: HTMLElement) =>
			codeblockCodeElement.style.setProperty(
				"max-height",
				`calc(${Math.ceil(
					codeblockCodeElement.scrollHeight + 0.01
				)}px + var(--code-padding) * ${
					codeblockCodeElement.classList.contains(
						"execute-code-output"
					)
						? "3.5 + var(--header-separator-width)"
						: "2"
				})`
			)
		);
	...
	if (typeof fold === "undefined")
		...
		fold
			? codeblockPreElement.classList.add("code-styler-folded")
			: codeblockPreElement.classList.remove("code-styler-folded");
...
function getPreClasses(
	...
	if (dynamic) {
		...
		if (codeblockParameters.lineUnwrap.alwaysEnabled)
			preClassList.push(
				codeblockParameters.lineUnwrap.activeWrap
					? "unwrapped-inactive"
					: "unwrapped"
			);
...
function decorateCodeblockLines(
	...
	let indentation = 0;
	getCodeblockLines(codeblockCodeElement, sourcePath, plugin).forEach(
		(line, index, codeblockLines) => {
			...
			if (index !== codeblockLines.length - 1)
				insertLineWrapper(
					...
					index + 1,
					line,
					plugin.settings.currentTheme.settings.codeblock.lineNumbers
				);
		}
	);
}
...
function getCodeblockLines(
			...
			if (ancestors.length >= 2) {
				codeblockHTML = codeblockHTML.replace(
					...
					ancestors.slice(1).reduce((result, element) => {
						...
						const splitTag =
							toHtml(elementCopy).split(/(?<=>)(?=<\/)/);
						return splitTag.splice(-1) + result + splitTag.join("");
					}, "<br>")
				);
			} else codeblockHTML = codeblockHTML.replace(/\n/, "<br>");
		}
	});
	...
	visitParents(splitHtmlTree, ["element"], (node) => {
		if (
			...
			node.properties?.className?.includes("comment")
		) {
	...
	if (codeblockLines.length === 1) codeblockLines = ["", ""];
	codeblockCodeElement.innerHTML = "";
	...
	const newChildren = linkMatches
		.reduce(
			(
				...
				const linkText = commentText.slice(
					linkMatch.index,
					linkMatch.index + linkMatch[0].length
				);
				...
				const linkChild = (
					fromHtml(linkContainer.innerHTML, { fragment: true })
...
export const executeCodeMutationObserver = new MutationObserver((mutations) => {
	mutations.forEach((mutation: MutationRecord) => {
		if (
			...
			(mutation.target as HTMLElement).tagName === "PRE"
		) {
			// Add execute code output
			const executeCodeOutput = (
				mutation.target as HTMLElement
			).querySelector("pre > code ~ code.language-output") as HTMLElement;
