...
import {
	...
	Display,
	CodeStylerSettings,
	CodeStylerThemeColours,
	FOLD_PLACEHOLDER,
	PARAMETERS,
	DEFAULT_SETTINGS,
	LANGUAGE_NAMES,
	LANGUAGES,
	SETTINGS_SOURCEPATH_PREFIX,
	EXAMPLE_INLINE_CODE,
	EXAMPLE_CODEBLOCK_PARAMETERS,
	EXAMPLE_CODEBLOCK_CONTENT,
	EXCLUDED_LANGUAGES,
	WHITELIST_CODEBLOCKS,
	INBUILT_THEMES,
	CodeStylerTheme,
} from "./Settings";
...
export class SettingsTab extends PluginSettingTab {
	...
	emptySettings(containerEl: HTMLElement) {
		...
		containerEl.createEl("h1", {
			text: "Settings for the Code Styler Plugin.",
		});
	}

	generateSettings(containerEl: HTMLElement) {
		if (this.page === "main") this.displayMainSettings(containerEl);
	...
	generateCodeblockStylingSwitcher(containerEl: HTMLElement) {
		new Setting(containerEl)
			...
			.setDesc(
				"Change dropdown to modify styles and settings of different codeblock sections"
			)
			.addDropdown((dropdown) =>
				dropdown
	...
	generateCoreSettings(containerEl: HTMLElement) {
		new Setting(containerEl)
			...
			.setDesc(
				"If enabled, styling will be applied when exporting to PDF."
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				...
				themeDropdown.onChange((value) => {
					...
					this.plugin.settings.currentTheme = structuredClone(
			...
			.addExtraButton((button) => {
				...
				button.onClick(() => {
					this.plugin.settings.themes[
						this.plugin.settings.selectedTheme
					] = structuredClone(this.plugin.settings.currentTheme);
					this.updateAlternativeHighlights();
					new Notice(
						`${this.plugin.settings.selectedTheme} theme saved successfully!`
					); //NOSONAR
					this.saveSettings();
				});
			})
			.addExtraButton((button) => {
				...
				button.onClick(() => {
					if (this.plugin.settings.selectedTheme.trim().length === 0)
						...
						this.plugin.settings.selectedTheme in
						DEFAULT_SETTINGS.themes
					)
						new Notice(
							"You cannot delete the default themes"
						); //NOSONAR
					else {
						delete this.plugin.settings.themes[
							this.plugin.settings.selectedTheme
						];
						new Notice(
							`${this.plugin.settings.selectedTheme} theme deleted successfully!`
						); //NOSONAR
			...
			.addExtraButton((button) => {
				...
				button.onClick(() => {
					if (this.plugin.settings.newTheme.trim().length === 0)
						...
						this.plugin.settings.newTheme in DEFAULT_SETTINGS.themes
					)
						new Notice(
							"You can't overwrite the default themes"
						); //NOSONAR
					else {
						if (
							this.plugin.settings.newTheme in
							this.plugin.settings.themes
						) {
							this.plugin.settings.themes[
								this.plugin.settings.newTheme
							] = structuredClone(
								this.plugin.settings.currentTheme
							);
							new Notice(
								`${this.plugin.settings.newTheme} theme updated successfully!`
							); //NOSONAR
						} else {
							this.plugin.settings.themes[
								this.plugin.settings.newTheme
							...
							new Notice(
								`${this.plugin.settings.newTheme} theme saved successfully!`
							); //NOSONAR
						}
	...
	generateCodeblockBodySettings(containerEl: HTMLElement) {
		...
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].codeblock
							.backgroundColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].codeblock.backgroundColour = saveColour;
					}
				);
			});
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].codeblock.textColour = saveColour;
					}
				);
			});
		new Setting(containerEl)
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						.setLimits(0, 25, 1)
						.setValue(
							this.plugin.settings.currentTheme.settings.codeblock
								.curvature
						)
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.codeblock.curvature =
								value;
							this.saveSettings();
						});
				});
					...
					button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.codeblock.curvature =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.codeblock.curvature;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.codeblock.curvature
							);
							this.saveSettings();
						});
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If enabled, languages with icons display a left border with the colour of the icon. The slider sets the width of the border."
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.settings.currentTheme.settings.advanced
							.languageBorderColour
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.advanced.languageBorderColour =
							value;
						this.disableableComponents[
							"languageBorderColour"
						].forEach((component) => {
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.advanced.languageBorderWidth =
								value;
							this.saveSettings();
						});
					this.disableableComponents["languageBorderColour"].push(
						resettableSlider
					);
				});
					...
					resetButton = button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.advanced.languageBorderWidth =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.advanced.languageBorderWidth;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.advanced.languageBorderWidth
							);
							this.saveSettings();
						});
					this.disableableComponents["languageBorderColour"].push(
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.codeblock.unwrapLines =
							value;
	...
	generateWrapLineSettings() {
		...
		new Setting(this.wrapLinesContainer)
			...
			.setDesc(
				"If enabled, in reading mode, holding click on a codeblock will wrap the lines for better visibility."
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.codeblock.wrapLinesActive =
							value;
						this.saveSettings(true);
					})
			);
	...
	generateCodeblockLineNumberSettings() {
		...
		new Setting(this.lineNumbersContainer)
			...
			.then((setting) => {
				this.createPickr(
					...
					"gutter_background",
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].gutter
							.backgroundColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].gutter.backgroundColour = saveColour;
					}
				);
			});
		new Setting(this.lineNumbersContainer)
		...
		new Setting(this.lineNumbersContainer)
			...
			.setDesc(
				"If enabled, highlights will also highlight the line numbers."
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.settings.currentTheme.settings.gutter
							.highlight
					)
					.setDisabled(
						!this.plugin.settings.currentTheme.settings.codeblock
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.gutter.highlight =
							value;
						this.saveSettings();
					})
			);
		new Setting(this.lineNumbersContainer)
			...
			.setDesc(
				"If enabled, the current line number in codeblocks will be indicated with a separate colour."
			)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.gutter.activeLine =
							value;
						this.saveSettings();
					})
			)
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].gutter.activeTextColour = saveColour;
					}
				);
			});
	}
	...
	generateCodeblockHeaderSettings(containerEl: HTMLElement) {
		...
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					"header_background",
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].header
							.backgroundColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].header.backgroundColour = saveColour;
					}
				);
			});
		new Setting(containerEl)
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						.setLimits(6, 32, 1)
						.setValue(
							this.plugin.settings.currentTheme.settings.header
								.fontSize
						)
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.header.fontSize =
								value;
							this.saveSettings();
						});
				});
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Style the header title text using bold and italic toggles, by setting a font or by setting a text colour."
			)
			.addToggle((toggle) => {
				toggle
					.setTooltip("Toggle bold title text")
					.setValue(
						this.plugin.settings.currentTheme.settings.header.title
							.textBold
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.title.textBold =
							value;
						this.saveSettings();
					});
			})
			.addToggle((toggle) => {
				toggle
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.title.textItalic =
							value;
						this.saveSettings();
					});
			})
			.addText((text) => {
				text.setPlaceholder("Font")
					.setValue(
						this.plugin.settings.currentTheme.settings.header.title
							.textFont
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.title.textFont =
							value;
						this.saveSettings();
					});
			})
			.then((setting) => {
				this.createPickr(
					...
					"header_title_text",
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].header.title
							.textColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].header.title.textColour = saveColour;
					}
				);
			});
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Title placeholder text for folded code when no title parameter is set."
			)
			.addText((text) =>
				text
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.foldPlaceholder =
							value;
						this.saveSettings();
						clearTimeout(foldPlaceholderTimeout);
						foldPlaceholderTimeout = setTimeout(
							() => this.rerender(),
							1000
						);
					})
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Colour of the line separating the codeblock header and the codeblock."
			)
			.then((setting) => {
				this.createPickr(
					this.plugin,
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Determine when to show language tags in the header. "Title Only" will only show language tags when the title parameter is set.'
			)
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange((value: Display) => {
						this.plugin.settings.currentTheme.settings.header.languageTag.display =
							value;
		...
		new Setting(containerEl)
			...
			.setDesc(
				'Determine when to show language icons where available. "Title Only" will only show language tags when the title parameter is set.'
			)
			.addDropdown((dropdown) =>
				dropdown
					.addOptions(DISPLAY_OPTIONS)
					.setValue(
						this.plugin.settings.currentTheme.settings.header
							.languageIcon.display
					)
					.onChange((value: Display) => {
						this.plugin.settings.currentTheme.settings.header.languageIcon.display =
							value;
	...
	generateHeaderTagSettings() {
		...
		if (
			this.plugin.settings.currentTheme.settings.header.languageTag
				.display === "none"
		)
			return;
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].header.languageTag.backgroundColour = saveColour;
					}
				);
			});
		new Setting(this.headerTagsContainer)
			...
			.setDesc(
				"Style the header language tag text using bold and italic toggles, by setting a font or by setting a text colour."
			)
			.addToggle((toggle) =>
				toggle
					...
					.setValue(
						this.plugin.settings.currentTheme.settings.header
							.languageTag.textBold
					)
					.setDisabled(
						this.plugin.settings.currentTheme.settings.header
							.languageTag.display === "none"
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.languageTag.textBold =
							value;
						this.saveSettings();
					})
			)
			.addToggle((toggle) =>
				toggle
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.languageTag.textItalic =
							value;
						this.saveSettings();
					})
			)
			.addText((text) =>
				text
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.languageTag.textFont =
							value;
						this.saveSettings();
					})
			)
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].header.languageTag.textColour = saveColour;
					}
				);
			});
	}
	...
	generateHeaderLanguageIconSettings() {
		...
		if (
			this.plugin.settings.currentTheme.settings.header.languageIcon
				.display === "none"
		)
			return;
		new Setting(this.headerIconsContainer)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.header.languageIcon.displayColour =
							value;
						this.saveSettings();
					})
			);
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.advanced.iconSize =
								value;
							this.saveSettings();
						});
				});
	...
	generateHeaderExternalReferenceSettings() {
		...
		this.headerExternalReferenceContainer.createEl("h5", {
			text: "External Reference Indicators Appearance",
		});
		new Setting(this.headerExternalReferenceContainer)
			.setName("Display Repository")
			.setDesc(
				"Display repository in codeblock header for external references."
			)
			.addToggle((toggle) =>
				toggle
					...
					.setValue(
						this.plugin.settings.currentTheme.settings.header
							.externalReference.displayRepository
					)
					.onChange((value) => {
			...
			.then((setting) => {
				this.createPickr(
					...
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].header
							.externalReference.displayRepositoryColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
					) => {
						relevantThemeColours[
							getCurrentMode()
						].header.externalReference.displayRepositoryColour =
							saveColour;
					},
					() =>
						!this.plugin.settings.currentTheme.settings.header
		...
		new Setting(this.headerExternalReferenceContainer)
			...
			.setDesc(
				"Display repository version in codeblock header for external references."
			)
			.addToggle((toggle) =>
				toggle
					...
					.setValue(
						this.plugin.settings.currentTheme.settings.header
							.externalReference.displayVersion
					)
					.onChange((value) => {
			...
			.then((setting) => {
				this.createPickr(
					...
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].header
							.externalReference.displayVersionColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
					) => {
						relevantThemeColours[
							getCurrentMode()
						].header.externalReference.displayVersionColour =
							saveColour;
					},
					() =>
						!this.plugin.settings.currentTheme.settings.header
		...
		new Setting(this.headerExternalReferenceContainer)
			...
			.setDesc(
				"Display the timestamp at which the reference was last updated."
			)
			.addToggle((toggle) =>
				toggle
					...
					.setValue(
						this.plugin.settings.currentTheme.settings.header
							.externalReference.displayTimestamp
					)
					.onChange((value) => {
			...
			.then((setting) => {
				this.createPickr(
					...
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].header
							.externalReference.displayTimestampColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
					) => {
						relevantThemeColours[
							getCurrentMode()
						].header.externalReference.displayTimestampColour =
							saveColour;
					},
					() =>
						!this.plugin.settings.currentTheme.settings.header
	...
	generateCodeblockHighlightSettings(containerEl: HTMLElement) {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If enabled, highlights the active line inside codeblocks."
			)
			.setClass("code-styler-spaced")
			.addToggle((toggle) => {
				return toggle
					.setValue(
						this.plugin.settings.currentTheme.settings.highlights
							.activeCodeblockLine
					)
					.onChange((value) => {
			...
			.then((setting) => {
				this.createPickr(
					...
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].highlights
							.activeCodeblockLineColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
					) => {
						relevantThemeColours[
							getCurrentMode()
						].highlights.activeCodeblockLineColour = saveColour;
					},
					() =>
						!this.plugin.settings.currentTheme.settings.highlights
							.activeCodeblockLine
		...
		this.disableableComponents["codeblockActiveLineHighlight"].push(
			this.pickrs["codeblock_active_line_highlight"].resetButton
		);
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].highlights.defaultColour = saveColour;
					}
				);
			});
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Define a new alternative highlight name. The colour can be modified after it is added."
			)
			.addText((value) => {
			...
			.addButton((button) => {
				...
				button.onClick(() => {
					if (this.plugin.settings.newHighlight.trim() === "")
						...
						!/^[^\d]\w*$/.test(this.plugin.settings.newHighlight)
					)
						new Notice(
							`"${this.plugin.settings.newHighlight}" is not a valid colour name.`
						); //NOSONAR
					else if (
						...
						new Notice(
							"Cannot override the default highlight parameter."
						); //NOSONAR
					else if (
						PARAMETERS.includes(
							this.plugin.settings.newHighlight
						...
						new Notice(
							"Cannot use other default parameters."
						); //NOSONAR
					else if (
						this.plugin.settings.newHighlight in
						this.plugin.settings.currentTheme.colours.light
							.highlights.alternativeHighlights
					)
						new Notice(
							`A highlight with the name "${this.plugin.settings.newHighlight}" already exists.`
						); //NOSONAR
					//TODO (@mayurankv) Future: Focus on existing highlighter - `renderMatches`
					else {
						const newColour = getRandomColour();
						this.plugin.settings.currentTheme.colours.light.highlights.alternativeHighlights[
							this.plugin.settings.newHighlight
						] = newColour;
						this.plugin.settings.currentTheme.colours.dark.highlights.alternativeHighlights[
							this.plugin.settings.newHighlight
						] = newColour;
						this.updateAlternativeHighlights();
						new Notice(
							`Added highlight "${this.plugin.settings.newHighlight}".`
						); //NOSONAR
						this.plugin.settings.newHighlight = "";
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If enabled, highlights fade away to the right. The slider sets the gradient colour stop as a percentage."
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
					...
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.advanced.gradientHighlights =
							value;
						this.disableableComponents[
							"gradientHighlighting"
						].forEach((component) => {
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						.setLimits(0, 100, 1)
						.setValue(
							+this.plugin.settings.currentTheme.settings.advanced.gradientHighlightsColourStop.slice(
								0,
								-1
							)
						)
					...
					this.disableableComponents["gradientHighlighting"].push(
						resettableSlider
					);
				});
				let resetButton: ExtraButtonComponent;
				setting.addExtraButton((button) => {
					resetButton = button
						...
						.setTooltip("Restore default colour stop")
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.advanced.gradientHighlightsColourStop =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.advanced.gradientHighlightsColourStop;
							resettableSlider.setValue(
								+this.plugin.settings.currentTheme.settings.advanced.gradientHighlightsColourStop.slice(
									0,
									-1
	...
	generateCodeblockLanguageSettings(containerEl: HTMLElement) {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Define languages in a comma separated list which the plugin should not decorate. You can use a wildcard (*) either at the beginning, or at the end. For example: ad-* will exclude codeblocks where the language starts with ad- e.g.: ad-info, ad-error etc."
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Define languages in a comma separated list which the plugin should style despite being processed by another plugin. You can use a wildcard (*) either at the beginning, or at the end. For example: ad-* will exclude codeblocks where the language starts with ad- e.g.: ad-info, ad-error etc."
			)
			.addText((text) =>
				text
					...
					.onChange((value) => {
						this.plugin.settings.processedCodeblocksWhitelist =
							value;
						this.saveSettings();
	...
	generateInlineCodeSettings(containerEl: HTMLElement) {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If enabled, in reading mode, inline code will be syntax highlighted based on a language set with `{language} highlighted_inline_code`. See the README for more information."
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.settings.currentTheme.settings.inline
							.syntaxHighlight
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.inline.syntaxHighlight =
							value;
						this.saveSettings(true);
					})
			);
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.settings.currentTheme.settings.inline.style
					)
					.onChange((value) => {
						this.plugin.settings.currentTheme.settings.inline.style =
							value;
		...
		new Setting(this.inlineCodeStylesContainer)
			...
			.then((setting) => {
				this.createPickr(
					...
					"title_text",
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].inline
							.titleTextColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].inline.titleTextColour = saveColour;
					}
				);
			});
		new Setting(this.inlineCodeStylesContainer)
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.titleFontWeight =
								value;
							this.saveSettings();
						});
				});
					...
					button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.inline.titleFontWeight =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.inline.titleFontWeight;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.inline.titleFontWeight
							);
							this.saveSettings();
						});
				});
	...
	generateInlineCodeStyleSettings() {
		...
		if (!this.plugin.settings.currentTheme.settings.inline.style) return;
		new Setting(this.inlineCodeStylesContainer)
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].inline.backgroundColour = saveColour;
					}
				);
			});
		new Setting(this.inlineCodeStylesContainer)
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].inline.activeTextColour = saveColour;
					}
				);
			});
		new Setting(this.inlineCodeStylesContainer)
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.fontWeight =
								value;
							this.saveSettings();
						});
				});
			...
			.then((setting) => {
				...
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.curvature =
								value;
							this.saveSettings();
						});
				});
		...
		new Setting(this.inlineCodeStylesContainer)
			...
			.setDesc(
				"Determines how much vertical inner padding in pixels inline code has."
			)
			.then((setting) => {
				let resettableSlider: SliderComponent;
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.paddingVertical =
								value;
							this.saveSettings();
						});
				});
					...
					button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.inline.paddingVertical =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.inline.paddingVertical;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.inline.paddingVertical
							);
							this.saveSettings();
						});
				});
		...
		new Setting(this.inlineCodeStylesContainer)
			...
			.setDesc(
				"Determines how much horizontal inner padding in pixels inline code has."
			)
			.then((setting) => {
				let resettableSlider: SliderComponent;
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.paddingHorizontal =
								value;
							this.saveSettings();
						});
				});
					...
					button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.inline.paddingHorizontal =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.inline.paddingHorizontal;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.inline.paddingHorizontal
							);
							this.saveSettings();
						});
				});
		...
		new Setting(this.inlineCodeStylesContainer)
			...
			.setDesc(
				"Determines how much horizontal outer margin is added to the inline code in pixels."
			)
			.then((setting) => {
				let resettableSlider: SliderComponent;
				setting.addSlider((slider) => {
					resettableSlider = slider
						...
						.setDynamicTooltip()
						.onChange((value) => {
							this.plugin.settings.currentTheme.settings.inline.marginHorizontal =
								value;
							this.saveSettings();
						});
				});
					...
					button
						...
						.onClick(() => {
							this.plugin.settings.currentTheme.settings.inline.marginHorizontal =
								this.plugin.settings.themes[
									this.plugin.settings.selectedTheme
								].settings.inline.marginHorizontal;
							resettableSlider.setValue(
								this.plugin.settings.currentTheme.settings
									.inline.marginHorizontal
							);
							this.saveSettings();
						});
				});
	...
	generateAdvancedHeading(containerEl: HTMLElement) {
		const advancedSettingsHeading = containerEl.createEl("h2", {
			text: "Advanced Settings",
			cls: `advanced-settings-header${
				this.hideAdvanced ? " header-folded" : ""
			}`,
		});
		advancedSettingsHeading.addEventListener("click", () => {
			this.hideAdvanced = !this.hideAdvanced;
			advancedSettingsHeading.classList.toggle(
				"header-folded",
				this.hideAdvanced
			);
			this.generateAdvancedSettings();
	...
	generateAdvancedSettings() {
		...
		new Setting(this.advancedSettingsContainer)
			...
			.setDesc(
				"If enabled, highlights the active line outside codeblocks."
			)
			.setClass("code-styler-spaced")
			.addToggle((toggle) =>
				toggle
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].highlights.activeEditorLineColour = saveColour;
					},
					() =>
						!this.plugin.settings.currentTheme.settings.highlights
							.activeEditorLine
		...
		new Setting(this.advancedSettingsContainer)
			...
			.setDesc(
				"If enabled, external references will be updated automatically on load when possible."
			)
			.addToggle((toggle) =>
				toggle
					.setTooltip("Toggle auto-update external references")
					.setValue(
						this.plugin.settings.externalReferenceUpdateOnLoad
					)
					.onChange((value) => {
						this.plugin.settings.externalReferenceUpdateOnLoad =
							value;
		...
		this.disableableComponents["editorActiveLineHighlight"].push(
			this.pickrs["editor_active_line_highlight"].resetButton
		);
		new Setting(this.advancedSettingsContainer)
			.setName("Reset inbuilt themes")
			.setDesc(
				"This will return all inbuilt themes to the plugin defaults"
			)
			.addButton((button) =>
				button.setButtonText("Reset").onClick(() => {
					Object.entries(INBUILT_THEMES).forEach(
						([themeName, theme]: [string, CodeStylerTheme]) =>
							(this.plugin.settings.themes[themeName] =
		...
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					"default_highlight",
					(relevantThemeColours: CodeStylerThemeColours) =>
						relevantThemeColours[getCurrentMode()].advanced
							.buttonColour,
					(
						relevantThemeColours: CodeStylerThemeColours,
						saveColour: Colour
						...
						relevantThemeColours[
							getCurrentMode()
						].advanced.buttonColour = saveColour;
					}
				);
			});
		new Setting(containerEl)
			...
			.then((setting) => {
				this.createPickr(
					...
					(
						...
						relevantThemeColours[
							getCurrentMode()
						].advanced.buttonActiveColour = saveColour;
					}
				);
			});
	}
	...
	generateExampleCodeblockSettings(containerEl: HTMLElement) {
		new Setting(containerEl)
			...
			.setDesc(
				"Parameters and language which would follow codeblock delimiter."
			)
	...
	generateExampleCodeblock() {
		...
		const codeblockString =
			"```````" +
			this.plugin.settings.exampleCodeblockParameters +
			"\n" +
		...
		MarkdownRenderer.render(
			this.plugin.app,
			codeblockString,
			this.exampleCodeblockContainer,
			SETTINGS_SOURCEPATH_PREFIX + codeblockString,
			this.plugin
		);
		this.exampleCodeblockContainer
			.querySelector("pre > button.copy-code-button")
			?.classList?.add("code-styler-settings-button");
	}
	...
	generateExampleInlineCode() {
		...
		MarkdownRenderer.render(
			this.plugin.app,
			"`" + this.plugin.settings.exampleInlineCode + "`",
			this.exampleInlineCodeContainer,
			SETTINGS_SOURCEPATH_PREFIX,
			this.plugin
		);
		this.exampleInlineCodeContainer
			.querySelector("code")
			?.classList?.add("code-styler-settings-inline-code");
	}
	...
	createPickr(
		...
		setting: Setting,
		id: string,
		getRelevantThemeColour: (
			relevantThemeColours: CodeStylerThemeColours
		) => Colour,
		saveRelevantThemeColour: (
			relevantThemeColours: CodeStylerThemeColours,
			saveColour: Colour
		) => void,
		disabled?: () => boolean
	) {
		const pickr: PickrResettable = new PickrResettable(
			plugin,
			containerEl,
			setting,
			getRelevantThemeColour,
		...
		pickr
			...
			.on(
				"save",
				(colour: Pickr.HSVaColor, instance: PickrResettable) => {
			...
			.on("cancel", (instance: PickrResettable) => {
				instance.hide();
			});
		setting.addExtraButton((button) => {
			pickr.resetButton = button
				.setIcon("reset")
	...
	rerender() {
		...
		if (this.page === "codeblock") this.generateExampleCodeblock();
		else if (this.page === "inline") this.generateExampleInlineCode();
	}
	...
	updateAlternativeHighlights() {
		...
		Object.keys(
			this.plugin.settings.currentTheme.colours.light.highlights
				.alternativeHighlights
		).forEach((alternativeHighlightName) => {
			new Setting(this.alternativeHighlightsContainer)
				.setName(alternativeHighlightName)
				.setDesc(
					`To highlight lines with this highlight, use the ${alternativeHighlightName} parameter.`
				)
				.then((setting) => {
					this.createPickr(
						this.plugin,
						...
						(relevantThemeColours: CodeStylerThemeColours) =>
							alternativeHighlightName in
							relevantThemeColours.light.highlights
								.alternativeHighlights
								? relevantThemeColours[getCurrentMode()]
										.highlights.alternativeHighlights[
										alternativeHighlightName
								  ]
								: this.plugin.settings.currentTheme.colours[
										getCurrentMode()
								  ].highlights.alternativeHighlights[
										alternativeHighlightName
								  ],
					...
					setting.addExtraButton((button) => {
						button
							...
							.onClick(() => {
								delete this.plugin.settings.currentTheme.colours
									.light.highlights.alternativeHighlights[
									alternativeHighlightName
								];
								delete this.plugin.settings.currentTheme.colours
									.dark.highlights.alternativeHighlights[
									alternativeHighlightName
								];
								new Notice(
									`Removed highlight "${alternativeHighlightName}".`
								); //NOSONAR
								this.updateAlternativeHighlights();
								this.saveSettings();
							});
	...
	redirectLanguages() {
		Object.entries(this.plugin.settings.redirectLanguages).forEach(
			([languageName, languageSettings]: [
				string,
				{ colour?: Colour; icon?: string }
			]) => {
				if ("icon" in languageSettings) {
					if (
						LANGUAGE_NAMES[languageName] in
						this.plugin.languageIcons
					)
						URL.revokeObjectURL(
							this.plugin.languageIcons[
					...
					this.plugin.languageIcons[LANGUAGE_NAMES[languageName]] =
						URL.createObjectURL(
							new Blob(
								[
									`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32">${languageSettings.icon}</svg>`,
								],
								{ type: "image/svg+xml" }
							)
						);
...
class PickrResettable extends Pickr {
	...
	constructor(
		...
		const settings: Pickr.Options = {
			...
			theme: "nano",
			default: getColour(
				getRelevantThemeColour(plugin.settings.currentTheme.colours)
			),
			...
			i18n: {
				...
				"btn:toggle":
					typeof tooltip !== "undefined" ? tooltip : "Select colour",
		...
		this.saveColour = (saveColour: Colour) => {
			saveRelevantThemeColour(
				plugin.settings.currentTheme.colours,
				saveColour
			);
			(async () => {
		...
		this.resetColour = () => {
			const resetColour: Colour = getRelevantThemeColour(
				plugin.settings.themes[plugin.settings.selectedTheme].colours
			);
...
function getCssVariable(cssVariable: CSS): HEX {
	...
	if (typeof variableValue === "string" && variableValue.startsWith("#"))
		...
		return `#${ColorTranslator.toHEXA(
			variableValue.replace(/calc\((.*?)\)/g, (match, capture) =>
				calc(capture)
			)
		).substring(1)}`;
	...
	const operators: { [key: string]: (num1: number, num2: number) => number } =
		{
			"+": (num1: number, num2: number): number =>
				Math.max(num1 + num2, 0),
			"-": (num1: number, num2: number): number =>
				Math.max(num1 - num2, 0),
		};
	if (splitString.length === 3)
		if (splitString[1] in operators)
			return `${operators[splitString[1]](
...
function getCurrentMode() {
	...
	if (body !== null) {
		...
		else if (body.classList.contains("theme-dark")) return "dark";
	}
