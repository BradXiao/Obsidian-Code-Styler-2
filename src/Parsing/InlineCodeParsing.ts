...
export function parseInlineCode(codeText: string): {
	parameters: InlineCodeParameters | null;
	text: string;
} {
	const match =
		/^{((?:[^"'{}\\]|\\.|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')*)} *?([^ ].*)$/.exec(
...
function parseInlineCodeParameters(
	...
	inlineCodeParameters.language = parameterLine
		.slice(0, languageBreak !== -1 ? languageBreak : parameterLine.length)
		.toLowerCase();
	if (languageBreak === -1) return inlineCodeParameters;
	const parameterStrings = parameterLine
		.slice(languageBreak + 1)
		.match(/(?:[^\s"']+|"[^"]*"|'[^']*')+/g);
	if (!parameterStrings) return inlineCodeParameters;
	parameterStrings.forEach((parameterString) =>
		parseInlineCodeParameterString(parameterString, inlineCodeParameters)
	);
	return inlineCodeParameters;
}

function parseInlineCodeParameterString(
	parameterString: string,
	inlineCodeParameters: InlineCodeParameters
): void {
	if (
		parameterString.startsWith("title:") ||
		parameterString.startsWith("title=")
	) {
		const titleMatch = /(["']?)([^\1]+)\1/.exec(
