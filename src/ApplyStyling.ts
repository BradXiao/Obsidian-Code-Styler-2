...
export function updateStyling(settings: CodeStylerSettings, app: App): void {
	...
	styleTag.innerText = (
		styleThemeColours(settings.currentTheme.colours) +
		styleThemeSettings(settings.currentTheme.settings, currentTheme) +
		styleLanguageColours(
			settings.currentTheme.settings,
			settings.redirectLanguages,
			currentTheme
		)
	)
		.trim()
...
function styleThemeColours(themeColours: CodeStylerThemeColours): string {
	return Object.keys(
		themeColours.light.highlights.alternativeHighlights
	).reduce(
		(result: string, alternativeHighlight: string) => {
			return (
				result +
				`
			body.code-styler .code-styler-line-highlighted-${alternativeHighlight
				.replace(/\s+/g, "-")
				.toLowerCase()} {
				--gradient-background-colour: var(--code-styler-${alternativeHighlight
					.replace(/\s+/g, "-")
					.toLowerCase()}-highlight-colour) !important;
			}
...
function getThemeColours(themeModeColours: CodeStylerThemeModeColours): string {
	return Object.entries({
		"codeblock-background-colour":
			themeModeColours.codeblock.backgroundColour,
		...
		...Object.entries(
			...
			(
				result: Record<string, Colour>,
				[alternativeHighlight, colour]: [string, Colour]
			): Record<string, Colour> => {
				result[
					`${alternativeHighlight
						.replace(/\s+/g, "-")
						.toLowerCase()}-highlight-colour`
				] = colour;
				return result;
			},
			{}
		...
		(result: string, [cssVariable, colour]: [string, Colour]): string => {
	...
	return `
		body.code-styler .code-styler-header-language-tag {
			...
			--code-styler-header-language-tag-text-italic: ${
				themeSettings.header.languageTag.textItalic
					? "italic"
					: "normal"
			};
			font-family: ${
		...
		body.code-styler {
			...
			--code-styler-inline-padding-vertical: ${
				themeSettings.inline.paddingVertical
			}px;
			...
			${
				themeSettings.header.languageIcon.displayColour
					? ""
		...
		${
			...
			.markdown-source-view :not(pre.code-styler-pre) > .code-styler-header-container {
				...
				--header-separator-width-padding: calc(var(--header-separator-width) - ` + //@ts-expect-error Does Exist
				  THEME_STYLES[currentTheme].border.size +
				  `px);
...
function styleLanguageColours(
	...
	return Object.entries(LANGUAGE_NAMES).reduce(
		(
			...
			if (
				LANGUAGES?.[languageDisplayName]?.colour ||
				(languageName in redirectLanguages &&
					"colour" in redirectLanguages[languageName])
			) {
				result += `
				.language-${languageName} {
					--language-border-colour: ${
						redirectLanguages?.[languageName]?.["colour"] ??
						LANGUAGES[languageDisplayName].colour
					};
					--language-border-width: ${
						themeSettings.advanced.languageBorderColour
							? themeSettings.advanced.languageBorderWidth
							: 0
					}px;
				}
...
function addThemeSettingsClasses(themeSettings: CodeStylerThemeSettings): void {
	themeSettings.inline.style
		? document.body.classList.add("code-styler-style-inline")
		: document.body.classList.remove("code-styler-style-inline");
	themeSettings.gutter.highlight
		? document.body.classList.add("code-styler-gutter-highlight")
		: document.body.classList.remove("code-styler-gutter-highlight");
	themeSettings.gutter.activeLine
	...
	document.body.classList.remove(
		...
		"code-styler-active-line-highlight-editor"
	); //TODO (@mayurankv) Is this section necessary? Is this function necessary?
	if (
		themeSettings.highlights.activeEditorLine &&
		themeSettings.highlights.activeCodeblockLine
	)
