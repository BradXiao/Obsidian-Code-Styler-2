...
import {
	...
	isCodeblockIgnored,
	isLanguageIgnored,
} from "./Parsing/CodeblockParsing";
import {
	InlineCodeParameters,
	parseInlineCode,
} from "./Parsing/InlineCodeParsing";
import {
	createHeader,
	createInlineOpener,
	getLanguageIcon,
...
export function createCodeblockCodeMirrorExtensions(
	...
	const ignoreListener = EditorView.updateListener.of(
		(update: ViewUpdate) => {
			//TODO (@mayurankv) Can I make this startState only? Does it need to be?
			const livePreviewExtensions = livePreviewCompartment.get(
				update.state
			);
			const toIgnore = isSourceMode(update.state);
			const fileIgnore =
				isFileIgnored(update.state) &&
				!(
					Array.isArray(livePreviewExtensions) &&
					livePreviewExtensions.length === 0
				);
			const fileUnignore =
				!toIgnore &&
				!isFileIgnored(update.state) &&
				...
				livePreviewExtensions.length === 0;
			if (
				isSourceMode(update.startState) !== toIgnore ||
				fileIgnore ||
				fileUnignore
			) {
	...
	const ignoreFileListener = EditorView.updateListener.of(
		(update: ViewUpdate) => {
			const ignoreExtensions = ignoreCompartment.get(update.state);
			const fileIgnore =
				isFileIgnored(update.state) &&
				!(
					Array.isArray(ignoreExtensions) &&
			...
			if (fileIgnore || fileUnignore)
				update.view.dispatch({
					effects: ignoreCompartment.reconfigure(
						fileIgnore ? [] : inlineDecorations
					),
				});
		}
	);
	...
	const settingsState = StateField.define<SettingsState>({
		create(): SettingsState {
			return {
				...
				processedCodeblocksWhitelist:
					settings.processedCodeblocksWhitelist,
			};
		},
		update(value: SettingsState): SettingsState {
			if (
				value.processedCodeblocksWhitelist !==
					settings.processedCodeblocksWhitelist ||
				value.excludedLanguages !== settings.excludedLanguages
			)
				return {
					excludedLanguages: settings.excludedLanguages,
					processedCodeblocksWhitelist:
	...
	const charWidthState = StateField.define<number>({
		...
		update(value: number, transaction: Transaction): number {
			return (
				transaction.state.field(editorEditorField)
					.defaultCharacterWidth * 1.105
			);
		},
	});
	const headerDecorations = StateField.define<DecorationSet>({
		...
		update(value: DecorationSet, transaction: Transaction): DecorationSet {
			return buildHeaderDecorations(transaction.state, (position) =>
				isFolded(transaction.state, position)
			);
		},
	...
	const foldDecorations = StateField.define<DecorationSet>({
		create(state: EditorState): DecorationSet {
			//TODO (@mayurankv) Can I change this?
				...
				codeblockFoldCallback(
					...
					(foldStart, foldEnd) => {
						builder.add(
							...
							foldEnd.to,
							foldDecoration(
								(iter.value as Decoration).spec.widget
		...
		update(value: DecorationSet, transaction: Transaction): DecorationSet {
			value = value
				...
				.update({ filter: (from: number, to: number) => from !== to });
			value = value.update({
				add: transaction.effects
					.filter(
						(effect) => effect.is(fold) || effect.is(unhideFold)
					)
					.map((effect) => foldRegion(effect.value)),
			}); //TODO (@mayurankv) Can I remove `, sort: true`
			transaction.effects
				.filter((effect) => effect.is(unfold) || effect.is(hideFold))
				.forEach(
					(effect) =>
						(value = value.update(unfoldRegion(effect.value)))
				);
			transaction.effects
				.filter((effect) => effect.is(removeFold))
				.forEach(
					(effect) =>
						(value = value.update(
							removeFoldLanguages(effect.value)
						))
				);
			return value;
		},
	...
	const hiddenDecorations = StateField.define<DecorationSet>({
		...
		update(value: DecorationSet, transaction: Transaction): DecorationSet {
			...
			value = value.update({
				add: transaction.effects
					.filter((effect) => effect.is(hideFold))
					.map((effect) => effect.value),
			}); //TODO (@mayurankv) Can I remove `, sort: true`
			transaction.effects
				.filter((effect) => effect.is(unhideFold))
				.forEach(
					(effect) =>
						(value = value.update(unhideFoldUpdate(effect.value)))
				);
			transaction.effects
				.filter((effect) => effect.is(removeFold))
				.forEach(
	...
	function settingsChangeExtender() {
		return EditorState.transactionExtender.of((transaction) => {
			...
			if (
				...
				const initialExcludedCodeblocks = codeblockProcessors.filter(
					(lang) =>
						!initialSettings.processedCodeblocksWhitelist
							.split(",")
							.map((lang) => lang.trim())
				...
				const currentExcludedCodeblocks = codeblockProcessors.filter(
					(lang) =>
						!settings.processedCodeblocksWhitelist
							.split(",")
							.map((lang) => lang.trim())
				...
				removeFoldLanguages = removeFoldLanguages.concat(
					setDifference(
			...
			if (
				initialSettings.excludedLanguages !== settings.excludedLanguages
			) {
				const initialExcludedLanguages =
					initialSettings.excludedLanguages
						.split(",")
						.map((lang) => lang.trim());
				const currentExcludedLanguages = settings.excludedLanguages
					.split(",")
					.map((lang) => lang.trim());
				removeFoldLanguages = removeFoldLanguages.concat(
					setDifference(
			...
			if (readdFoldLanguages.length !== 0)
				addEffects = addEffects.concat(
					convertReaddFold(transaction, readdFoldLanguages)
				);
			...
			(transaction: Transaction) => {
				...
				const foldDecorationsState =
					transaction.startState
						...
						?.map(transaction.changes) ?? Decoration.none;
				const hiddenDecorationsState =
					transaction.startState
						.field(hiddenDecorations, false)
						?.map(transaction.changes) ?? Decoration.none;
				transaction.newSelection.ranges.forEach(
					(range: SelectionRange) => {
						...
						for (
							let iter = hiddenDecorationsState.iter();
							iter.value !== null;
							iter.next()
						) {
	...
	class LineNumberWidget extends WidgetType {
		...
		eq(other: LineNumberWidget): boolean {
			return (
				...
				this.codeblockParameters.lineNumbers.alwaysEnabled ===
					other.codeblockParameters.lineNumbers.alwaysEnabled &&
				this.codeblockParameters.lineNumbers.alwaysDisabled ===
					other.codeblockParameters.lineNumbers.alwaysDisabled &&
				this.codeblockParameters.lineNumbers.offset ===
					other.codeblockParameters.lineNumbers.offset &&
				this.maxLineNum === other.maxLineNum &&
		...
		toDOM(): HTMLElement {
			return createSpan({
				attr: {
					style:
						this.maxLineNum.toString().length >
						(
							this.lineNumber +
							this.codeblockParameters.lineNumbers.offset
						).toString().length
	...
	class CommentLinkWidget extends WidgetType {
		...
		toDOM(): HTMLElement {
			const linkParentElement = createDiv({
				attr: { class: "code-styler-comment-link" },
			});
	...
	class HeaderWidget extends WidgetType {
		...
		constructor(
			...
			this.sourcePath = sourcePath;
			this.plugin = plugin;
			this.iconURL = getLanguageIcon(
				this.codeblockParameters.language,
				this.plugin.languageIcons
			);
			this.folded = folded;
			this.hidden = isHeaderHidden(
				this.codeblockParameters,
				this.themeSettings,
				this.iconURL
			);
		}
		...
		eq(other: HeaderWidget): boolean {
			return (
				this.codeblockParameters.language ===
					other.codeblockParameters.language &&
				this.codeblockParameters.title ===
					other.codeblockParameters.title &&
				this.codeblockParameters.reference ===
					other.codeblockParameters.reference &&
				this.codeblockParameters.fold.enabled ===
					other.codeblockParameters.fold.enabled &&
				this.codeblockParameters.fold.placeholder ===
					other.codeblockParameters.fold.placeholder &&
				this.themeSettings.header.foldPlaceholder ===
					other.themeSettings.header.foldPlaceholder &&
				this.themeSettings.header.languageIcon.display ===
					other.themeSettings.header.languageIcon.display &&
				this.themeSettings.header.languageTag.display ===
					other.themeSettings.header.languageTag.display &&
		...
		toDOM(view: EditorView): HTMLElement {
			...
			if (this.codeblockParameters.language !== "")
				headerContainer.classList.add(
					`language-${this.codeblockParameters.language}`
				);
			if (this.folded)
	...
	class OpenerWidget extends WidgetType {
		...
		eq(other: OpenerWidget): boolean {
			return (
				this.inlineCodeParameters.language ==
					other.inlineCodeParameters.language &&
				this.inlineCodeParameters.title ==
					other.inlineCodeParameters.title &&
				this.inlineCodeParameters.icon ==
					other.inlineCodeParameters.icon &&
				getLanguageIcon(
					this.inlineCodeParameters.language,
					this.plugin.languageIcons
				) ==
					getLanguageIcon(
						other.inlineCodeParameters.language,
						other.plugin.languageIcons
					)
			);
		}

		toDOM(): HTMLElement {
			return createInlineOpener(
				this.inlineCodeParameters,
				this.plugin.languageIcons,
				["code-styler-inline-opener", "cm-inline-code"]
			);
		}
	}
	...
	function buildHeaderDecorations(
		state: EditorState,
		foldValue: (position: number, defaultFold: boolean) => boolean = (
			position,
			defaultFold
		) => defaultFold
	) {
		...
		syntaxTree(state).iterate({
			enter: (syntaxNode) => {
				if (syntaxNode.type.name.includes("HyperMD-codeblock-begin")) {
					...
					codeblockParameters = parseCodeblockParameters(
						trimParameterLine(startLine.text.toString()),
					...
					if (
						...
						if (
							!SPECIAL_LANGUAGES.some((regExp) =>
								new RegExp(regExp).test(
									codeblockParameters.language
								)
							)
						)
	...
	function buildLineDecorations(state: EditorState): DecorationSet {
		...
		for (
			...
			const codeblockParameters =
				iter.value.spec.widget.codeblockParameters;
			const showLineNumbers =
				(settings.currentTheme.settings.codeblock.lineNumbers &&
			...
			const lineNumberMargin =
				maxLineNum.toString().length > 2
					? maxLineNum.toString().length * state.field(charWidthState)
					: undefined;
			builder.add(
				foldStart.from,
				foldStart.from,
				Decoration.line({
					attributes: {
						style: `--line-number-gutter-width: ${
							lineNumberMargin
								? lineNumberMargin + "px"
								: "calc(var(--line-number-gutter-min-width) - 12px)"
						};`,
						class:
							"code-styler-line" +
							(["^$"]
								.concat(SPECIAL_LANGUAGES)
								.some((regExp) =>
									new RegExp(regExp).test(
										codeblockParameters.language
									)
								...
								? ""
								: ` language-${codeblockParameters.language}`),
					},
				})
			);
			if (showLineNumbers)
			...
			for (let i = foldStart.number + 1; i <= state.doc.lines; i++) {
				...
				if (codeblockParameters.language === "markdown") continue;
				convertCommentLinks(
					state,
					line,
					sourcePath,
		...
		syntaxTree(state).iterate({
			enter: (syntaxNode) => {
				if (syntaxNode.type.name.includes("comment_hmd-codeblock")) {
					const commentText = state.sliceDoc(
						syntaxNode.from,
						syntaxNode.to
					);
					const linkMatches = [
					...
					linkMatches.forEach((linkMatch: RegExpMatchArray) => {
						if (typeof linkMatch?.index === "undefined") return;
						...
						if (
							...
							state.selection.ranges.some(
								(range: SelectionRange) =>
									rangeInteraction(from, to, range)
							)
						) {
							const mdBreak = linkMatch[0].indexOf("](");
							//TODO (@mayurankv) Add editor wide viewer to allow clicking on files with cursor inside
							if (mdBreak === -1) {
								const wikilinkSeparator =
									linkMatch[0].indexOf("|");
								builder.add(
									from + 2,
									to - 2,
									Decoration.mark({
										class: "cm-hmd-internal-link code-styler-source-link",
										attributes: {
											destination: linkMatch[0].slice(
												2,
												wikilinkSeparator !== -1
													? wikilinkSeparator
													: -2
								...
								builder.add(
									...
									from + mdBreak,
									Decoration.mark({
										class: "cm-link code-styler-source-link",
										attributes: {
											destination: linkMatch[0].slice(
												mdBreak + 2,
												-1
		...
		syntaxTree(state).iterate({
			enter: (syntaxNode) => {
				...
				if (parameters.value === null)
					addUnstyledInlineDecorations(
						state,
						builder,
						parameters as { from: number; to: number; value: null },
						text,
						section,
						sourceMode
					);
				else
					addStyledInlineDecorations(
						state,
						builder,
						parameters as {
							from: number;
	...
	function addStyledInlineDecorations(
		state: EditorState,
		builder: RangeSetBuilder<Decoration>,
		parameters: { from: number; to: number; value: InlineCodeParameters },
		...
		if (
			sourceMode ||
			state.selection.ranges.some((range: SelectionRange) =>
				rangeInteraction(section.from, section.to, range)
			)
		)
			builder.add(
				parameters.from,
				parameters.to,
				Decoration.mark({ class: "code-styler-inline-parameters" })
			);
		else {
			builder.add(parameters.from, parameters.to, Decoration.replace({}));
			if (
				parameters.value?.title ||
				(parameters.value?.icon &&
					getLanguageIcon(
						parameters.value.language,
						plugin.languageIcons
					))
			)
				builder.add(
					parameters.from,
					parameters.from,
					Decoration.replace({
						widget: new OpenerWidget(parameters.value, plugin),
					})
				);
		}
		...
		modeHighlight(
			{
				start: parameters.to,
				text: text.value,
				language: parameters.value.language,
			},
			builder
		);
	}
	...
	function convertReaddFold(
		...
		for (
			...
			//TODO (@mayurankv) Refactor: Try and make this startState
			if (
				!iter.value.spec.widget.codeblockParameters.fold.enabled ||
				!readdLanguages.includes(
					iter.value.spec.widget.codeblockParameters.language
				)
			)
				continue;
			codeblockFoldCallback(
				...
				(foldStart, foldEnd) => {
					addEffects.push(
						fold.of({
							...
							value: {
								spec: {
									language: (iter.value as Decoration).spec
										.widget.codeblockParameters.language,
								},
							},
						})
					);
	...
	function documentFold(
		...
		for (
			...
			if (iter.value.spec.widget.hidden) continue;
			...
			codeblockFoldCallback(iter.from, state, (foldStart, foldEnd) => {
				if (
					...
					addEffects.push(
						unfold.of({ from: foldStart.from, to: foldEnd.to })
					);
			});
		}
...
const fold: StateEffectType<{
	from: number;
	to: number;
	value: { spec: { language: string } };
}> = StateEffect.define();
const unfold: StateEffectType<{ from: number; to: number }> =
	StateEffect.define();
...
function codeblockFoldCallback(
	...
	if (foldEnd !== null) foldCallback(foldStart, foldEnd);
}
...
function getInlineCodeRanges(
	...
	parameters: {
		from: number;
		to: number;
		value: InlineCodeParameters | null;
	};
	text: { from: number; to: number; value: string };
	section: { from: number; to: number };
} | null {
	const delimiterSize = getInlineDelimiterSize(syntaxNode);
	if (delimiterSize === null) return null;
	const inlineCodeText = state.doc.sliceString(
		syntaxNode.from,
		syntaxNode.to
	);
	...
	return {
		parameters: {
			...
			to: syntaxNode.from + parametersLength,
			value: parameters,
		},
		text: {
			from: syntaxNode.from + parametersLength + 1,
			to: syntaxNode.to,
			value: text,
		},
		section: {
...
function addUnstyledInlineDecorations(
	state: EditorState,
	builder: RangeSetBuilder<Decoration>,
	parameters: { from: number; to: number; value: null },
	text: { from: number; to: number; value: string },
	section: { from: number; to: number },
	sourceMode: boolean
) {
	if (text.value) {
		if (
			!state.selection.ranges.some(
				(range: SelectionRange) =>
					range.to >= section.from && range.from <= section.to
			) &&
			!sourceMode
		)
			builder.add(parameters.from, parameters.to, Decoration.replace({}));
...
function modeHighlight(
	{
		...
		text,
		language,
	}: { start: number; text: string; language: string },
	builder: RangeSetBuilder<Decoration>
) {
	//@ts-expect-error Undocumented Obsidian API
	const mode = window.CodeMirror.getMode(
		window.CodeMirror.defaults,
		window.CodeMirror.findModeByName(language)?.mime
	);
		...
		while (!stream.eol()) {
			...
			if (style)
				builder.add(
					start + stream.start,
					start + stream.pos,
					Decoration.mark({ class: `cm-${style}` })
				);
...
export function editingDocumentFold(view: EditorView, toFold?: boolean) {
	view.dispatch({
		effects: foldAll.of(
			typeof toFold !== "undefined" ? { toFold: toFold } : {}
		),
	});
	view.requestMeasure();
}
...
function foldOnClick(
	...
	codeblockFoldCallback(
		...
		(foldStart, foldEnd) => {
			view.dispatch({
				effects: foldLines(!folded, {
					from: foldStart.from,
					to: foldEnd.to,
					value: { spec: { language: language } },
				}),
			});
			view.requestMeasure();
		}
...
function unfoldRegion({
	...
	return {
		filter: (from: number, to: number) => to <= foldFrom || from >= foldTo,
		filterFrom: foldFrom,
		filterTo: foldTo,
	};
}
	...
	if (typeof filePath !== "undefined")
		return (
			this.app.metadataCache
				.getCache(filePath)
				?.frontmatter?.["code-styler-ignore"]?.toString() === "true"
		);
	return false;
}
