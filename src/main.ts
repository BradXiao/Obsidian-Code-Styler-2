...
import {
	createCodeblockCodeMirrorExtensions,
	editingDocumentFold,
} from "./EditingView";
import {
	destroyReadingModeElements,
	readingDocumentFold,
	executeCodeMutationObserver,
	readingViewCodeblockDecoratingPostProcessor,
	readingViewInlineDecoratingPostProcessor,
} from "./ReadingView";
import {
...
export default class CodeStylerPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.languageIcons = Object.keys(LANGUAGES).reduce(
		...
		}); // Add codeblock decorating markdownPostProcessor
		this.registerMarkdownPostProcessor(async (el, ctx) => {
			await readingViewInlineDecoratingPostProcessor(el, ctx, this);
		}); // Add inline code decorating markdownPostProcessor
			...
			callback: () => {
				...
				if (activeView) {
					if (activeView.getMode() === "preview")
						...
						editingDocumentFold(
							activeView.editor.cm.docView.view,
							true
						);
				}
			},
		...
		this.addCommand({
			...
			callback: async () => {
				await updateExternalReferencedFiles(
					this,
